...
export default {
  ...
  computed: {
    ...
    //将来提交订单选中的最终地址
    userDefaultAddress() {
      //find:找到第一个符合条件的元素，返回第一个符合条件的元素，如果没有符合条件的元素，则返回undefined
      return this.addressInfo.find((item) => item.isDefault == 1) || {};
    },
    //总共多少件商品
    // totalNum(){
    //   return this.orderInfo.detailArrayList.reduce((total,item)=>{
    ...
    // totalPrice(){
    //   return this.orderInfo.detailArrayList.reduce((total,item)=>{
    //     return total + (item.orderPrice * item.skuNum);
    //   },0);
    // }
  },
  methods: {
